...
class gpsSignal:
    ...
    code_phase = None       # initial code phase offset for each satellite
    ...
    def __init__(self, sv, fs=sys_.fs, fi=sys_.fi, carrier_phase=None,
                 code_phase=None, doppler=None, doppler_drift=None, data=None):
        '''
        ...
        if code_phase is None:
            self.code_phase = np.zeros(self.len_sv)
        else:
            if len(code_phase) != self.len_sv:
    ...
    def generate(self, duration, t0=0, has_ca=True):
        '''
        Generate gps
        ...
        for i in range(self.len_sv):
            ...
            _, ca_signal = caCode(self.sv[i]).generate_ca_signal(duration, t0, self.code_phase[i], self.doppler[i],
                                                                 self.doppler_drift[i], self.fs, sys_.f_ca / self.fi)
            # carrier wave
            _, fi_signal = self.generate_carrier_signal(duration, t0, self.carrier_phase[i],
                                                        self.doppler[i], self.doppler_drift[i])
            # data
            t, data_signal = self.generate_data_signal(duration, t0, self.data[i], self.doppler[i],
                                                       self.doppler_drift[i], sys_.f_data / self.fi)
            if has_ca:
                signal += ca_signal * fi_signal * data_signal
            else:
                signal += fi_signal * data_signal
        return t, signal

    def generate_data_signal(self, duration, t0=0, data=None, doppler=0, doppler_drift=0, k=sys_.k_data):
        if data is None:
            data = [0, 1]

        t = np.arange(t0, t0 + duration, 1 / self.fs)
        # phase =  f_ca + f_d + d_fd * t, where
        # f_d = k * doppler, dfd = k * doppler_drift
        phase = t * (sys_.f_data + k * (doppler + doppler_drift * t))
        t_code = np.mod(np.floor(phase), len(data)).astype(np.int)
        signal = 2 * np.array(data)[t_code] - 1   # convert from 0/1 to -1/1
        return t, signal

    def generate_carrier_signal(self, duration, t0, carrier_phase=0, doppler=0, doppler_drift=0):
        t = np.arange(t0, t0 + duration, 1 / self.fs)
...
class gpsSignalAnimation:
    ...
    def __init__(self, sv, duration, code_phase, doppler, doppler_drift):
    ...
    def _animate(self, i):
        ...
        for k in range(self.len_sv):
            _, ca_signal = self.ca_codes[k].generate_ca_signal(sys_.T_ca, t0=i * sys_.T_ca)
...
if __name__ == '__main__':
    ...
    code_phase = [0, 250, 0]
    doppler = [0, 0, 0]
    doppler_drift = [0, 0, 5e3]
    duration = 0.1

    # Animation
    gps_corr_anim = gpsSignalAnimation(sv, duration, code_phase, doppler, doppler_drift)
    gps_corr_anim.start(save_gif=True)
