...
class carrierLoop:
    ...
    def track(self, signal, duration, t0):
        t, nco_cos, nco_sin = self._generate_carrier_signals(duration, t0)
        # coherent integration
        i = np.sum(signal * nco_cos)
        q = np.sum(signal * nco_sin)
        error = self._discriminator(i, q)
        filter_output = self._filter(error)
        self._update_filter_status()
        self.filter_output_history[:-1] = self.filter_output_history[1:]
        self.filter_output_history[-1] = filter_output
        return t, nco_cos, nco_sin, filter_output

    def _generate_carrier_signals(self, duration, t0):
    ...
    def _update_filter_status(self):
        ...
        if self.filter_status == FLL:
            ...
            if np.std(self.filter_output_history) < 2:
                self.cnt_fll2pll += 1
            else:
                self.cnt_fll2pll = 0
            # change to PLL after stabilized
            if self.cnt_fll2pll > 10:
                self.filter_status = PLL
...
class carrierLoopAnimation:
    ...
    def init_gps_signal_generator(self, sv=None, fi=sys_.fi, code_phase=None, doppler=None, doppler_drift=None):
        if sv is None:
            sv = [1]
        self.gps_signal_generator = gpsSignal(sv, fs=self.fs, fi=fi, code_phase=code_phase,
                                              carrier_phase=[np.pi / 2], doppler=doppler, doppler_drift=doppler_drift)
    ...
    def _init_animation(self):
        ...
        self.axes[0].set_title('GPS signal (display first 10 chips)')
...
if __name__ == '__main__':
    ...
    carrier_loop_anim.start(save_gif, f_name='carrier_loop_ani1')

    # animation 2:
    carrier_loop_anim = carrierLoopAnimation(T_coh, duration=0.1, fs=fs)
    carrier_loop_anim.init_gps_signal_generator(fi=fi, doppler=[225], doppler_drift=[1e2])
    carrier_loop_anim.init_carrier_loop(fi, mode=1)     # PLL
    carrier_loop_anim.start(save_gif, f_name='carrier_loop_ani2')

    # animation 3:
    carrier_loop_anim = carrierLoopAnimation(T_coh, duration=0.12, fs=fs)
    carrier_loop_anim.init_gps_signal_generator(fi=fi, doppler=[225], doppler_drift=[1e2])
    carrier_loop_anim.init_carrier_loop(fi, mode=0)     # FLL-PLL
    carrier_loop_anim.start(save_gif, f_name='carrier_loop_ani3')
